...
export default function Sidebar({ title, menuItems, activeKey, onSelect, profile, onSignOut, onClose }) {
  return (
    <aside className="w-full max-w-full lg:w-[280px] lg:max-w-[280px] shrink-0">
      <div className="relative rounded-[28px] border border-slate-800 bg-slate-900/85 p-4 shadow-2xl shadow-black/30 backdrop-blur sm:p-6">
        <div className="mb-6 flex items-start justify-between gap-3">
          ...
          {onClose ? (
            <button
              type="button"
              className="lg:hidden rounded-2xl border border-slate-700 bg-slate-800 px-3 py-2 text-sm font-semibold text-slate-200 transition hover:bg-slate-700"
              onClick={onClose}
            >
              Close
            </button>
          ...
          {menuItems.map((item) => (
            <button
              ...
              className={`flex w-full items-center justify-between rounded-2xl px-4 py-3 text-left text-sm font-medium transition ${
                activeKey === item.key ? "bg-sky-600 text-white shadow-lg shadow-sky-600/20" : "text-slate-300 hover:bg-slate-800 hover:text-white"
              }`}
            >
        ...
        <div className="mt-8 rounded-3xl border border-slate-800 bg-slate-800/70 p-4 text-sm text-slate-400">
